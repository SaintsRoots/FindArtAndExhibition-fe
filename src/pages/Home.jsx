        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center min-h-screen">
          ...
          <div className="space-y-8">
            <div className="space-y-6">
              <div className="inline-flex items-center px-4 py-2 bg-purple-500/20 rounded-full border border-purple-500/30">
                <span className="text-purple-300 text-sm font-medium">Discover Amazing Art</span>
              </div>
          ...
          <div className="relative">
            <div className="relative overflow-hidden rounded-2xl bg-white/10 backdrop-blur-sm border border-white/20">
                  ...
                  >
                    ...
                    <div className="absolute bottom-6 left-6 right-6 text-white">
                      <div className="flex items-center gap-2 mb-2">
                        <span className="px-3 py-1 text-xs bg-purple-500 rounded-full">{artwork.badge}</span>
                        <span className="text-sm text-gray-300">{artwork.category}</span>
                      </div>
                      <h3 className="text-2xl font-bold mb-1">{artwork.title}</h3>
                      <p className="text-gray-300 mb-3">by {artwork.artist}</p>
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
                          <span className="text-2xl font-bold">{artwork.price} RWF</span>
                          {artwork.originalPrice && (
                            <span className="text-lg text-gray-400 line-through">{artwork.originalPrice} RWF</span>
                          )}
                        </div>
              ...
              {/* Carousel Controls */}
              <button
                onClick={() => setFeaturedIndex((prev) => (prev - 1 + featuredArtworks.length) % featuredArtworks.length)}
              ...
              <button
                onClick={() => setFeaturedIndex((prev) => (prev + 1) % featuredArtworks.length)}
      ...
      <div className="container mx-auto px-6 text-center">
        <div className="max-w-3xl mx-auto">
          ...
          <p className="text-xl text-purple-100 mb-8">
            Join thousands of art lovers and collectors who trust our platform to discover, connect, and acquire amazing artworks.
          </p>
